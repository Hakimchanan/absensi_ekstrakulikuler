...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    ...
    docReady(function () {
        ...
        function sendDataToServer(nim, status, date, time, keterangan) {
            fetch('insert_absen.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ nim, status, date, time, keterangan }),
            })
        ...
        function onScanSuccess(decodedText, decodedResult) {
            if (decodedText !== lastResult) {
                ...
                const status = "hadir";
                const keterangan = "ekstrakulikuler";

                resultContainer.innerHTML = `${decodedText}<br> ${formattedDate} <br> ${formattedTime} <br> ${status} <br> ${keterangan} <br>`;

...
if (!empty($search)) {
    ...
    $query .= " WHERE nama LIKE '%$search%' 
                OR waktu_absen LIKE '%$search%' 
                OR warga.nim LIKE '%$search%' 
                OR kamar LIKE '%$search%' 
                OR nama_pengurus LIKE '%$search%' 
                OR jenis_absen LIKE '%$search%' 
                OR keterangan LIKE '%$search%' 
                OR status_kehadiran LIKE '%$search%'";
    ...
    <div class="flex">
      ...
      <div class="flex-1 ml-64 p-6 overflow-scroll h-screen">
        ...
        <header class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-semibold text-gray-800">Absensi Ekstrakulikuler Warga Asrama UTM</h2>
        ...
        <div class="pdf_magrib bg-white p-6 rounded-lg shadow-lg mb-10">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-xl font-semibold text-gray-800">Ekstrakulikuler</h3>           
              <form method="get" action="">
                <input type="text" name="search" placeholder="Cari..." value="<?= htmlspecialchars($search); ?>" class="border border-gray-300 rounded px-3 py-1 text-gray-700 focus:outline-none focus:border-blue-500" />
                  <button type="submit" class="ml-2 bg-blue-500 text-white px-4 py-2 rounded">Cari</button>
              </form>
          </div>
          <table class="min-w-full bg-white border">
            <thead>
              <tr>
                ...
                <th class="py-2 px-3 bg-gray-200">Kegiatan</th>
            ...
            <tbody>
              <?php while ($row = mysqli_fetch_assoc($hasil)) { ?>
                  <tr class="border-b border-gray-200 hover:bg-gray-100">
                      ...
                      <td class="py-2 px-4">
                        <?php if ($row['keterangan'] == 'Banjari') {?>
                          Banjari
                        <?php } elseif ($row['keterangan'] == 'Tilawah') { ?>
                          Tilawah
                        <?php } elseif ($row['keterangan'] == 'Arduino Nano') { ?>
                          Arduino
                        <?php } elseif ($row['keterangan'] == 'Bengkel Seni') { ?>
                          Bengkel Seni
                        <?php } elseif ($row['keterangan'] == 'English Course') { ?>
                          English Course
                        <?php } elseif ($row['keterangan'] == 'Arabic') { ?>
                          Arabic
                        <?php } elseif ($row['keterangan'] == 'alpha') { ?>
                          <span class="text-red-700 bg-red-200 rounded-full px-3 py-1 text-xs font-semibold">EkstraTidakTerdeteksi</span>
                        <?php } else { ?>
                          <span class="text-red-700 bg-red-200 rounded-full px-3 py-1 text-xs font-semibold">EkstraTidakTerdeteksi</span>
                        <?php } ?>
                      </td>
    ...
    <script>
      function printPDF() {
        ...
        html2pdf()
          .set({
            margin: 0.5,
            filename: "Absensi_Ekstrakulikuler.pdf",

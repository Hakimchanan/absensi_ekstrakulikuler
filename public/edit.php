...
if (isset($_POST['submit'])) {
    ...
    if ($hasil) {
        header("Location: absensi_ekstra.php");
    } else {
...
<body class="bg-gray-100 flex justify-center items-center min-h-screen">
    <div class="w-full max-w-md bg-white p-8 rounded shadow-lg">
        ...
        <form action="" method="POST">
            ...
            <div class="mb-4">
                <label for="keterangan" class="block text-sm font-medium text-gray-700">Kegiatan</label>
                <select name="keterangan" id="keterangan" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                    <option value="Banjari" <?= $row['keterangan'] == 'Banjari' ? 'selected' : '' ?>>Banjari</option>
                    <option value="Tilawah" <?= $row['keterangan'] == 'Tilawah' ? 'selected' : '' ?>>Tilawah</option>
                    <option value="Arduino Nano" <?= $row['keterangan'] == 'Arduino Nano' ? 'selected' : '' ?>>Arduino Nano</option>
                    <option value="Bengkel Seni" <?= $row['keterangan'] == 'Bengkel Seni' ? 'selected' : '' ?>>Bengkel Seni</option>
                    <option value="English Course" <?= $row['keterangan'] == 'English Course' ? 'selected' : '' ?>>English Course</option>
                    <option value="Arabic" <?= $row['keterangan'] == 'Arabic' ? 'selected' : '' ?>>Arabic</option>
